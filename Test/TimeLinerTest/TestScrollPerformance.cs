...
public class TestScrollPerformance
{
    ...
    public void HiddenRow_ReenteringViewport_RefreshesExistingBindings()
    {
        ...
        var item = model.TimeLines[1].TimeLineItems[0];
        Border existingVisual = new() { DataContext = item };
    ...
    public void ExternalFile_ScrollBenchmark()
    {
        ...
        var (model, rowHeight) = CreateBenchmarkModel(assemblyPath);
        Type modelType = model.GetType();
        modelType.GetProperty("TimeLinesVisibleHeight").SetValue(model, 600d);
        var load = modelType.GetMethod("LoadAsync", new[] { typeof(string), typeof(double) })
        ...
        var setHorizontal = modelType.GetProperty("HorizontalScrollOffset").SetMethod.CreateDelegate<Action<double>>(model);
        var setVertical = modelType.GetProperty("VerticalScrollOffset").SetMethod.CreateDelegate<Action<double>>(model);
        var timelines = ((IEnumerable)modelType.GetProperty("TimeLines").GetValue(model)).Cast<object>().ToArray();
        ...
        ItemsControl rows = new() { Width = 1200, Height = 600, DataContext = model };
        ...
        Window window = new()
        {
            Content = rows, Width = 1220, Height = 640, ShowActivated = false,
            ShowInTaskbar = false, Left = -10000, Top = -10000
        };
        long notifications = 0;
        PropertyChangedEventHandler count = (_, _) => notifications++;
        var items = timelines.SelectMany(x => ((IEnumerable)x.GetType().GetProperty("TimeLineItems").GetValue(x))
            .Cast<INotifyPropertyChanged>()).ToArray();
        foreach (var item in items) item.PropertyChanged += count;
        try
        {
            window.Show();
            Flush(window);
    ...
    public class BenchmarkServiceProxy : DispatchProxy
    {
        ...
        {
            if (targetMethod.Name == "Load" && Settings != null) return Settings;
            if (targetMethod.Name == "Save" && Settings != null) { Settings = args[0]; return null; }
